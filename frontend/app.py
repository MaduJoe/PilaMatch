...
def render_auth_page():
    st.title("🧘 PilaMatch")
    st.subheader("필라테스 & 요가 강사 매칭 플랫폼")
...
def render_find_jobs_step():
    ...
    with filter_col1:
        category_filter = st.radio("종목", ["전체", "필라테스", "요가"], horizontal=True, label_visibility="collapsed")
    with filter_col2:
        region_filter = st.selectbox("지역", ["전체"] + list(SEOUL_REGIONS.keys()), label_visibility="collapsed")
...
def render_create_job_step():
    ...
    # 3. 지역 선택 + 지도
    st.subheader("3️⃣ 지역")
    col_region, col_map = st.columns([1, 2])
    with col_region:
        region = st.selectbox("서울", list(SEOUL_REGIONS.keys()), label_visibility="collapsed")
    with col_map:
        render_kakao_map(region, height=200)

    # 4. 시급 선택 (프리셋 버튼)
    st.subheader("4️⃣ 시급")
    rate_cols = st.columns(len(RATE_PRESETS))
    for i, (rate, label) in enumerate(RATE_PRESETS):
        with rate_cols[i]:
    ...
    if st.button("🚀 공고 등록하기", type="primary", use_container_width=True):
        try:
            ...
            # Reset form state
            for key in ["job_category", "job_type", "job_rate"]:
...
def render_offers_step():
    ...
    try:
        ...
        if pending:
            st.subheader(f"대기 중인 오퍼 ({len(pending)})")
            for offer in pending:
                with st.container():
                    col1, col2, col3 = st.columns([2, 1, 1])

                    with col1:
                        st.write(f"**제안 시급:** ₩{int(float(offer['proposed_rate'])):,}")
                        if offer.get("message"):
                            st.caption(offer["message"])

                    with col2:
                        if st.button("수락", key=f"accept_{offer['id']}", type="primary", use_container_width=True):
                            try:
                    ...
                    with col3:
                        if st.button("거절", key=f"reject_{offer['id']}", use_container_width=True):
                            try:
        ...
        # Accepted offers - can create contract
        accepted = [o for o in offers if o["status"] == "accepted"]
        if accepted:
            st.subheader(f"수락한 오퍼 ({len(accepted)}) - 계약 생성 대기")
            for offer in accepted:
                with st.container():
                    col1, col2 = st.columns([3, 1])
                    with col1:
                        st.write(f"**시급:** ₩{int(float(offer['proposed_rate'])):,}")
                    with col2:
                        if st.button("계약 생성", key=f"contract_{offer['id']}", type="primary", use_container_width=True):
                            try:
...
def render_applicants_step():
    ...
    try:
        ...
        for job in jobs:
            with st.expander(f"📋 {job['title']} ({job['date']}) - 지원자 {job.get('application_count', 0)}명"):
                st.write(f"**시급:** ₩{int(float(job['hourly_rate'])):,} | **유형:** {job['job_type']}")
                    ...
                    if not applications:
                        ...
                        for app in applications:
                            ...
                            with col1:
                                ...
                                st.write(f"**{instructor_name}** (경력 {exp_years}년)")
                                if rating:
                                    st.caption(f"⭐ {rating:.1f}")
                                if app.get("cover_letter"):
                                    st.caption(app["cover_letter"][:100])

                            with col2:
                                # Show offer status if offer was sent
                                if app.get("has_offer"):
                                    if app["status"] == "pending":
                                        st.caption("📨 오퍼 전송됨 (응답 대기)")
                                    ...
                                    st.caption(status_map.get(app['status'], app['status']))

                            with col3:
                                if app["status"] == "pending" and not app.get("has_offer"):
                                    if st.button("오퍼 보내기", key=f"offer_{app['id']}", use_container_width=True):
        ...
        if st.session_state.get("show_offer_modal"):
            ...
            with st.form("offer_form"):
                proposed_rate = st.number_input("제안 시급", min_value=0, step=5000, value=50000)
                message = st.text_area("메시지 (선택)")

                col1, col2 = st.columns(2)
                with col1:
                    if st.form_submit_button("오퍼 전송", type="primary"):
                        try:
                            client.create_offer({
...
def render_contracts_step():
    ...
    try:
        ...
        active = [c for c in contracts if c["status"] in ["confirmed", "in_progress"]]
        ...
        if active:
            ...
            for contract in active:
                status_emoji = {"confirmed": "📝", "in_progress": "🔵"}
                status_text = {"confirmed": "서명 대기", "in_progress": "진행 중"}

                with st.container():
                    st.write(f"{status_emoji.get(contract['status'], '')} **{status_text.get(contract['status'], contract['status'])}**")

                    col1, col2 = st.columns(2)
                    with col1:
                        st.caption(f"📅 {contract['date']}")
                        st.caption(f"⏰ {contract['start_time']} - {contract['end_time']}")
                    with col2:
                        st.caption(f"💰 ₩{int(float(contract['total_amount'])):,}")

                    # Show agreement terms for confirmed contracts
                    if contract["status"] == "confirmed":
                        ...
                        if agree:
                            if st.button("✍️ 서명하고 시작하기", key=f"sign_{contract['id']}", type="primary", use_container_width=True):
                                try:
                                    client.set_contract_in_progress(contract["id"])
                                    st.success("계약이 체결되었습니다! 수업 시작을 기다립니다.")
                                    st.balloons()
                                    st.rerun()
                                except APIError as e:
                                    st.error(f"오류: {e.message}")
                    ...
                    # In progress contracts
                        ...
                        with col_b:
                            if st.button("❌ 취소", key=f"cancel_btn_{contract['id']}", use_container_width=True):
                                st.session_state[f"show_cancel_{contract['id']}"] = True

                        with col_c:
                            if st.button("⚠️ 불참 신고", key=f"noshow_btn_{contract['id']}", use_container_width=True):
                        ...
                        if st.session_state.get(f"show_cancel_{contract['id']}"):
                            with st.form(key=f"cancel_form_{contract['id']}"):
                                ...
                                col1, col2 = st.columns(2)
                                with col1:
                                    if st.form_submit_button("취소 확정"):
                                        try:
        ...
        if completed:
            with st.expander(f"완료된 계약 ({len(completed)})"):
...
def render_complete_step():
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("새로운 일 찾기" if st.session_state.user["role"] == "instructor" else "새 공고 등록", use_container_width=True):
            st.session_state.page = "find_jobs" if st.session_state.user["role"] == "instructor" else "create_job"
            st.rerun()
    with col2:
...
def main():
    ...
    with col1:
        ...
        st.title(f"🧘 PilaMatch - {role_text}")
    with col2:
        if st.button("로그아웃"):
            logout()
            st.rerun()
